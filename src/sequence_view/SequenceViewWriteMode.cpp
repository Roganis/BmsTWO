...
SequenceView::Context *SequenceView::WriteModeContext::PlayingPane_MousePress(QMouseEvent *event)
{
	...
	if (notes.size() > 0){
		...
		}else if (event->button() == Qt::RightButton && iTime >= 0 && lane >= 0){
			// Right-click on an empty spot: place/toggle a sample "stop" — the
			// sounding sample's playback is cut at this tick. Works in both view
			// modes; clicking exactly on an existing stop marker removes it.
			sview->PlaceOrToggleSampleStop(sview->FindSoundingSampleChannelAtTime(iTime), iTime);
		}
	}
	return this;
}
